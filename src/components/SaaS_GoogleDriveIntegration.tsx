...
export default function SaaS_GoogleDriveIntegration({
  ...
  const handleSaveManualToken = (e: React.FormEvent) => {
    ...
    setSyncLogs(prev => [
      {
        id: `gdrive-manual-connect-${Date.now()}`,
        timestamp,
        integration: "Google Drive Sync",
    ...
    setUpdateSuccessMsg("Manually connected Google account via OAuth overrides!");
    setManualTokenVal("");
        ...
        <form onSubmit={handleSaveManualToken} className="bg-slate-900 text-white rounded-2xl p-4.5 border border-slate-800 space-y-3 text-left animate-fade-in">
          <div className="space-y-1">
            ...
            <p className="text-[10px] text-slate-400 font-semibold leading-relaxed font-sans">
              Provide a valid Google Access Token (e.g. from the Google OAuth Playground or a standalone login session) to immediately connect Google Drive without popup prompts.
            </p>
          </div>
          <div className="flex flex-col sm:flex-row gap-2">
            <input
              ...
              className="flex-grow bg-slate-950 border border-slate-850 text-xs px-3 py-2 rounded-xl text-emerald-400 font-mono focus:outline-none focus:ring-2 focus:ring-indigo-500/50"
            />
            <div className="flex gap-2">
              <button
                type="submit"
                className="flex-1 sm:flex-none px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-[10px] font-bold uppercase tracking-wider transition-all cursor-pointer"
              >
                Apply Token
              </button>
              <button
          ...
          {googleAccessToken ? (
            ...
            <div className="flex flex-col items-end gap-1.5">
              ...
              <button 
                type="button"
                onClick={() => setShowManualTokenInput(!showManualTokenInput)}
                className="text-[9px] text-slate-400 hover:text-indigo-505 font-bold uppercase tracking-wider select-none cursor-pointer hover:underline"
              >
